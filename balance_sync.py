...
def run_on_user(user_id):
    ...
    num_balance_days = 90
    start_date_str = (now() - datetime.timedelta(days=num_balance_days)).strftime(
        "%Y-%m-%d"
    )
    ...
    if response.status_code == 200:
        ...
        try:  # Wrapped in try catch because not passing validation causes errors
            ...
            log_this(
                f"MONGO DB OR OTHER ERROR ON USER {user_id} ON DAILY SYNC", "error"
            )
        ...
        log_this(
            f"        {user_id}: balances call failed for date range {start_date_str}-{end_date_str}",
            "warning",
        )
...
def main():
    handler = RotatingFileHandler(
        f"{logs_path}daily-balance-data-sync.log", "a+", (1000**2) * 200, 2
    )
    ...
    user_set_start_idx = int(
        (len(rows) * balance_sync_counter) / balance_sync_user_set_divisor
    )
    user_ids = [
        str(row[0])
    ...
    process_end = now()
    log_this(
        f"Balance Sync {user_set_length} users: {process_start} -> {process_end} | Total run time: {process_end-process_start}",
        "warning"
    )
    flush_log_buffer()
    logger.info("\n\n\n")
