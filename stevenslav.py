...
def handle_pave_request(
    ...
    logging.info(f"\tResponse: {res_code} \n{json.dumps(res.json())[:100]}...{json.dumps(res.json())[-100:]}")
...
def main():
    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            rows = conn.execute("SELECT DISTINCT id FROM public.users WHERE created_at >= (NOW() - INTERVAL '2 hours')").fetchall()
        ...
        for user_id in tqdm(user_ids):
            ...
            if len(sys.argv) == 1:
                ...
                for access_token in access_tokens:
                    res = requests.post(f"http://127.0.0.1:8123/v1/users/{user_id}/upload?num_transaction_days=3650", json={"access_token": f"{access_token}"})
                    res = res.json()
